#define BOT_VERSION "5.0"

// A beam search over turns, built on v4's one-turn machinery.
//
...
// States carried from one level to the next. Narrow on purpose: measured
// against v4.5, 2 and 4 both land around 66% and 8 and 16 fall off. What the
// budget buys is better turns along a line, not more lines.
static const int BEAM_WIDTH = 4; // @cg-param
// Turns the beam may look ahead. The clock usually stops it first; where it
// does not, a line 32 turns long assumes a frozen opponent for 32 turns, which
// is already well past what the assumption is worth.
static const int MAX_BEAM_DEPTH = 32; // @cg-param
...
public:
    ...
    int evaluated = 0;
    int depthReached = 0;
    // Value maps rebuilt and wishes solved this turn: the two costs that
    // decide how deep the beam gets.
    int rebuilds = 0, solves = 0, sweeps = 0;
...
private:
    ...
    // What each combination played on a state's board paid, and the ranking
    // over them. Written once per state, read once per disrupt.
    int playMine[MAX_COMBOS], playFoe[MAX_COMBOS], playValue[MAX_COMBOS];
    uint64_t playHash[MAX_COMBOS], playKey[MAX_COMBOS];
    uint16_t playCells[MAX_COMBOS][PAINT_PER_TURN];
    uint8_t playN[MAX_COMBOS];
    int playedCount = 0;
    ...
    struct State
    {
        ...
        uint8_t n;
        int16_t slot; // region disrupted, -1 for none
    };
    ...
    bool playCombos(int nodeIdx, int slot, uint64_t slotHash, int credit)
    {
        ...
        for (int c = 0; c < nPlayed; c++)
        {
            ...
            pushRails(cells, used);
            int gainMine = 0, gainFoe = 0;
            collectIncome(cells, used, gainMine, gainFoe);
            popFrame();
            evaluated++;

            playMine[playedCount] = gainMine;
            playFoe[playedCount] = gainFoe;
            playValue[playedCount] = bought;
            playHash[playedCount] = railHash;
            playN[playedCount] = (uint8_t)used;
            for (int i = 0; i < used; i++)
                playCells[playedCount][i] = (uint16_t)cells[i];
            // What the level will rank this child on, bar the constants its
            // parent and its disrupt add: enough to order them here.
            const long long key =
                (long long)(gainMine - gainFoe) * SCORE_WEIGHT + bought;
            playKey[playedCount] =
                ((uint64_t)(key + KEY_BIAS) << 20) |
                (uint64_t)(0xFFFFF - playedCount);
            playedCount++;
        }

        const int head = min(playedCount, EMIT_PER_DISRUPT);
        partial_sort(playKey, playKey + head, playKey + playedCount,
                     greater<uint64_t>());
        emitPlayed(nodeIdx, slot, slotHash, credit);
        return playedCount == nPlayed;
    }
    ...
    void emitPlayed(int nodeIdx, int slot, uint64_t slotHash, int credit)
    {
        ...
        {
            const int c = (int)(0xFFFFF - (playKey[i] & 0xFFFFF));
    ...
    void applyPath(int nodeIdx)
    {
        ...
        for (int i = pathLen; i-- > 0;)
        {
            ...
            // The disrupt first: a rail it would have erased was never kept
            // in the state, so replaying in this order rebuilds it exactly.
            pushDisrupt(s.slot);
            for (int j = 0; j < s.n; j++)
                cells[j] = s.cells[j];
            pushRails(cells, s.n);
    ...
    void pushRails(const int *cells, int n)
    {
        Frame f;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            owner[idx] = (int8_t)myId;
            net[idx] = 1;
            railUndo.push_back(idx);
        }
        frames.push_back(f);
    }
    ...
    void buildCombos()
    {
        ...
        nPlayed = curDepth <= 1 ? nCombos : min(nCombos, COMBO_PRUNING_WIDTH);
...
public:
    ...
    void gameTurn()
    {
        ...
        if (!actions.empty())
        {
            ...
            if (disrupt != -1)
                msg << " D" << disrupt;
            // How far the beam got: it is the one number a replay cannot show.
            msg << " d" << planner.depthReached;
        ...
        const long long us =
            std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                .count();
        fprintf(stderr,
                "turn : %lld us / %d ms  depth %d  %d boards  %d solves  "
                "%d sweeps  %d maps  %d rails (%d paint)  disrupt %d\n",
                us, PLAN_BUDGET_MS, planner.depthReached, planner.evaluated,
                planner.solves, planner.sweeps, planner.rebuilds,
