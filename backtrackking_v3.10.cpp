...
public:
    ...
    void plan(const Map &board, ActionSet &outAction, int &outDisrupt)
    {
        ...
        // After the rails: chooseRails may have spread the map to find them,
...
private:
    ...
    int chooseDisrupt(const Map &board)
    {
        ...
        // Candidates by score, best first, then the first one whose collateral
        // is acceptable. Ranking rather than taking the max: the top region can
        // be disqualified by what it would cost us, and the runner-up is then
        // the move -- a disrupt is free, so a second-best one still beats none.
        candidates.clear();
        ...
        for (const auto &cand : candidates)
            if (cutIsWorthIt(board, cand.second))
                return board.stat->regions[cand.second].id;
        return -1;
    }

    // Inking a region kills every connection its rails carry, ours included.
    // Weighing the rails inside the region is not enough: a connection is a
    // line across the board, and severing it erases the income of all of it.
    // So the whole of each affected connection is counted, on both sides, and
    // the cut is refused when it would cost us more than the opponent. A tie
    // is taken: the disrupt costs nothing and the instability still builds up.
    bool cutIsWorthIt(const Map &board, int slot) const
    {
        if (board.connWords == 0)
        {
            DBG_CUT_TALLY(slot, 0, 0);
            return true; // nothing live to break yet
        }

        int mine = 0, foe = 0;
        const uint64_t *row = &board.regionConnMask[(size_t)slot * board.connWords];
    ...
    void chooseRails(const Map &board, ActionSet &out)
    {
        ...
        {
            int idx = bestCell(board, paintLeft, out);
            // Nothing valued left in reach, and paint still to spend: bleed
            // the map one ring outwards and look again, rather than waste the
            // turn. Late games are almost entirely made of these turns, the
            // shortest paths being built long before the clock runs out.
            while (idx < 0 && diffuseValues(board))
                idx = bestCell(board, paintLeft, out);
            if (idx < 0)
                break;
