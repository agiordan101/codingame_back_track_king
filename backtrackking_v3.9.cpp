...
// Weight of one live connection crossing a rail, when scoring a disrupt. A
// rail on an active connection pays its owner every turn; one off the network
// is a bet. Counted as (1 + ACTIVE_CONN_WEIGHT * crossings).
static const int ACTIVE_CONN_WEIGHT = 2;
...
public:
    ...
    // How many live town-to-town connections cross each cell, from the
    // referee's partOfActiveConnections. Saturates at 255; kept apart from
    // Tile, which is full at 4 bytes, and streamed alongside it.
    vector<unsigned char> activeConnCount;
    ...
    void readTurnState(istream &in, map<pair<int, int>, bool> &outActiveConnections)
    {
        ...
        for (int y = 0; y < grid.height; y++)
        {
            for (int x = 0; x < grid.width; x++)
            {
                ...
                activeConnCount[y * grid.width + x] =
                    (unsigned char)min(activeHere, 255);
...
private:
    ...
    int chooseDisrupt(const Map &board)
    {
        ...
        for (int idx = 0; idx < N; idx++)
        {
            ...
            if (owner != foeId && owner != myId)
                continue;
            // A rail earning its owner income right now is worth more than an
            // idle one, on both sides of the subtraction: erasing ours costs
            // us that same income.
            const int weight =
                1 + ACTIVE_CONN_WEIGHT * (int)board.activeConnCount[idx];
            if (owner == foeId)
                regionScore[slot] += disruptValue[idx] * weight;
            else
                regionScore[slot] -= disruptValue[idx] * weight;
        }
