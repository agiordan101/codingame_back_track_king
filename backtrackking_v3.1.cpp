...
public:
    ...
    {
        buildValueMap(board);
        // One ring, always: the shortest-path corridors are one cell wide and
        // arbitrary among many equal paths, so scoring anything off them --
        // the opponent's rails above all -- needs the map bled outwards once.
        diffuseValues(board);
...
private:
    ...
    bool diffuseValues(const Map &board)
    {
        spread = value;
        bool changed = false;
