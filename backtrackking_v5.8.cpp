...
// sunk into the ones we invented. Swept at depth 4 over both pruning pools,
// 48 wins either way: b48 > b64 > b32 > b24.
static const int BEAM_WIDTH = 48; // @cg-param
// Turns the beam may look ahead -- a hard ceiling now, not a safety net. Six
// turns of a 56%-accurate prediction is already 0.56^6 of credibility. Four is
// the floor of that argument, not its limit: at three the beam runs out of
// tree before it runs out of clock, and the freed budget buys nothing.
static const int MAX_BEAM_DEPTH = 4; // @cg-param
...
static const int COMBO_PRUNING_WIDTH = 200; // @cg-param

// How fast that width falls away with depth. A line is a guess about an
// opponent we only half predict, and the guess compounds, so each level plays
// the one above's count divided by this, down to COMBO_FLOOR; 1 keeps v5.1's
// flat step. Softer decays were swept against this one and none of them
// separated: over a depth of four the ladder barely has room to bite, and it
// is the floor below that carries the deep levels.
static const int COMBO_DECAY = 2; // @cg-param
// Below this a level stops being a search and becomes a rollout, so the decay
// stops here rather than at one combination. This is the width every level
// past the second actually plays, which is why raising it from 32 mattered
// more than anything done to the pool above it.
static const int COMBO_FLOOR = 50; // @cg-param
...
// What a cell of the shortest-in-cells route is worth against a cell of the
// cheapest-in-paint one, in percent. The game pays the walk that crosses
// fewest cells, not the one that cost least to build, so once a connection is
// live the short way is the one worth holding -- but the cheap way is the one
// a turn can still afford to finish, so it stays ahead while it has free
// cells left. Under 100 the A* route keeps the lead until it fills up, and
// the switch then happens on its own: a full route offers nothing to buy.
// Swept against v5.6: 55 wins, and the curve falls away on either side
// (55 > 40 > 70 > 85), 100 being i4, which lost outright.
static const int SHORTCUT_REWARD_PCT = 55; // @cg-param

// What a neighbour of a valued cell is worth, as a divisor. The map only ever
// lit the corridors themselves, so a turn could not see a rail one cell off
// one -- the kind that reroutes a walk the opponent is paid for. Seeding every
// cell the corridors left at zero with a share of what surrounds it puts those
// within reach of the ranking. Four neighbours still outweigh a single one,
// which is what makes a junction outrank a dead end.
// The candidate pool is capped and full most turns, so a seeded cell does not
// add an option, it takes one: halving this to 2 floods the pool and collapses
// the bot to 18.5%. Four is the share that pays for the slot it costs.
static const int SEED_DIVISOR = 4; // @cg-param
...
private:
    ...
    vector<int32_t> ringUndo; // cells a state's rings raised, to clear on exit
    ...
    void syncValueMap()
    {
        ...
        valueInk = inkWords;
        valueValid = true;
        ringUndo.clear(); // the rebuilt map carries none of the old rings
        rebuilds++;
        ...
        for (int w = 0; w < nLinks; w++)
            if (addPathReward(linkSrc[w], linkDst[w]))
                addPathReward(linkSrc[w], linkDst[w], true,
                              SHORTCUT_REWARD_PCT);
        ...
        // One ring laid into the map for good, at a quarter rather than a
        // half: it is not meant to rival a corridor, only to lift the cells
        // beside one above the empty board so the ranking can reach them.
        // Part of the map, so it costs no undo and the cache still holds it.
        // Pinned in turn, so it is as untouchable as a corridor: the late-game
        // rings spread past it instead of overwriting it, and undoing one of
        // them cannot clear it.
        if (diffuseInto(value, false, SEED_DIVISOR))
            for (int idx = 0; idx < N; idx++)
                wasZero[idx] = (value[idx] == 0) ? 1 : 0;
    ...
    // Shortest path between two towns, ink impassable, every cell of it
    // raised to W+H-cost -- so a short connection, the kind a turn can
    ...
    // Stepping by cells rather than by paint finds the walk the game actually
    // pays; `pct` scales what that walk is worth. Returns true when the route
    // it found is already live end to end.
    bool addPathReward(int srcIdx, int dstIdx, bool byCells = false,
                       int pct = 100)
    {
        if (srcIdx == dstIdx)
            return false;
        ...
        while (!heap.empty())
        {
            ...
            for (int p = nbrAt[cur], e = nbrAt[cur + 1]; p < e; p++)
            {
                ...
                if (cellInked[nIdx])
                    continue;
                const int step = byCells ? 1 : cellCost[nIdx];
                if (cellCost[nIdx] == 0)
                    continue;
                const int ng = g + step;
        ...
        if (total < 0)
            return false; // walled apart by ink: nothing to steer towards

        // Never zero, so a path longer than the board still marks its cells.
        const int reward = max(1, (W + H - total) * pct / 100);
        // The best corridor a cell sits on, not the sum of them: adding them
        // up made crossings outrank every cell of the shortest connection,
        // and the pruning then saw only the intersections.
        bool linked = true;
        for (int cur = dstIdx; cur != -1; cur = parent[cur])
        {
            value[cur] = max(value[cur], baseValue[cur] + reward);
            if (!net[cur])
                linked = false;
        }
        return linked;
    ...
    bool diffuseInto(vector<int> &target, bool record = false, int div = 2)
    {
        spread = target;
        bool changed = false;
        for (int idx = 0; idx < N; idx++)
        {
            ...
            for (int p = nbrAt[idx], e = nbrAt[idx + 1]; p < e; p++)
                sum += spread[nbrList[p]] / div;
            if (sum > 0)
            {
                target[idx] = sum;
                changed = true;
                if (record)
                    ringUndo.push_back((int32_t)idx);
            }
        }
        return changed;
    }

    // A ring only ever raises cells the value map left at zero, so undoing one
    // is clearing those cells -- no old values to keep. The state that bled
    // the map hands it back as it found it, and the cache stays truthful.
    bool diffuseRing() { return diffuseInto(value, true); }

    void undoRings()
    {
        for (size_t i = ringUndo.size(); i-- > 0;)
            value[ringUndo[i]] = 0;
        ringUndo.clear();
    }
